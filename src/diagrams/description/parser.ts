...
function classifyArrow(arrow: string): ArrowStyle {
  switch (arrow) {
    case '->>': return { style: 'solid', arrowHead: 'filled' };
    case '-->':
    case '->':  return { style: 'solid', arrowHead: 'open' };
    case '--':  return { style: 'solid', arrowHead: 'none' };
    case '..>':
    case '.>':  return { style: 'dashed', arrowHead: 'open' };
    case '..':  return { style: 'dashed', arrowHead: 'none' };
    default:    return { style: 'solid', arrowHead: 'open' };
  }
}
...
const COMMANDS: readonly Command[] = [
  ...
  // 2. Ignored directives: skinparam, title, hide, show, direction
  {
    pattern: /^(?:skinparam|title|hide|show|left\s+to\s+right\s+direction|top\s+to\s+bottom\s+direction)\b/i,
    execute() { /* ignore */ },
  },

  // 3. Closing brace — pops the current container
  {
    pattern: /^\}\s*$/,
    execute(state) { state.containerStack.pop(); },
  },

  // 4. Business-actor shorthand: :Name:/ or :Name: /
  ...
  // 6. Business-usecase shorthand: (Name)/ or (Name) /
  {
    pattern: /^\(([^)]+)\)\s*\/\s*$/,
    execute(state, match) {
      const name = match[1]!.trim();
  ...
  // 7. Interface shorthand: () InterfaceName (standalone, no arrow)
  {
    pattern: /^\(\)\s+(\S+)\s*$/,
    execute(state, match) {
      const name = match[1]!.trim();
  ...
  // 8. Links — MUST come before bracket (9) and paren (10) shorthands.
  ...
  //    Arrows (longest first): ->> --> -> -- ..> .. .>
  {
    ...
    execute(state, match) {
      ...
      const { style, arrowHead } = classifyArrow(match[2]!);
      const { stereotype, label } = extractLinkStereotype((match[4] ?? '').trim());
      state.ast.links.push(buildLink({ from, to, style, arrowHead, label, stereotype }));
    },
  },

  // 9. Bracket shorthand: [Name] [as Alias] [<<stereotype>>] [#color]
  {
    pattern: /^\[([^\]]+)\](.*)?$/,
    execute(state, match) {
  ...
  // 10. Use-case shorthand: (Name) standalone
  {
    pattern: /^\(([^)]+)\)\s*$/,
    execute(state, match) {
      const name = match[1]!.trim();
  ...
  // 11. Container inline block: CONTAINER header { body }
  {
  ...
  // 12. Container open block: CONTAINER header {
  {
  ...
  // 13. Generic keyword dispatch: any KEYWORD_TO_SYMBOL key followed by a name.
