...
| ID | Gap | Scope |
|----|-----|-------|
| ~~F1~~ **FIXED** | Spline-clip edge-drop | ~~`layout-helpers.ts` clip-splicing can produce non-`1+3n` point counts that `buildDotPathFromSplinePoints` (`svek-edge-geometry.ts`) rejects; caller `try/catch` skips the edge. Affects 3 fixtures.~~ Fixed at origin: clipping is now a faithful port of upstream `DotPath#simulateCompound` + `XCubicCurve2D#subdivide` (`spline-clip.ts`) — it clips bezier-by-bezier (the invariant holds by construction) and finds the boundary by upstream's 8 midpoint subdivisions, discarding the straddling sliver, so the crossing point matches the jar's rather than a more-precise one. The earlier polyline clip, which treated the `1+3n` array as a flat vertex list, was the actual divergence. The whole description golden corpus (294 fixtures) now drops **zero** edges (was 3, across `berufi-69-dara369` +1 and `lirebi-26-voka556` +2). **Output change (additive):** those 3 previously-missing edges now render; no currently-rendering edge's geometry shifts — the golden survey confirmed every geometry-changing clip in the corpus was one that also dropped its edge, so there was no surviving-clip path to perturb. The per-edge `try/catch` in `renderer.ts#drawEdges` is retained as a general safety net, not as this bug's mitigation. |
| F2 | Structural features | ~~Legend, title/header/footer~~ **implemented 2026-07-13 (mission G0b**, `plans/g0b-annotations/`): title/caption/legend/header/footer render for every engine via the shared annotation chrome (`src/core/annotations/`, the `DiagramChromeFactory` port). Residual F2: `<img>`, monospace creole, and `mainframe` drawing (parsed; rendering deferred to G0's LimitFinder port — see DIVERGENCES.md § General). |
...
| ~~F4~~ **FIXED** | Multi-leaf document-dimension under-count | ~~1px under production~~ **Fixed 2026-07-13 (mission G0**, `plans/g0-limitfinder/`): `LimitFinder`/`UGraphicNo`/`MinMax` ported (`src/core/klimt/`), and description document sizing cut over to upstream SvekResult's recipe — ink walk + `delta(15,15)` + the `CucaDiagram` backwards-compat margin `(0,5,5,0)` (the margin was the actual 1px); single-leaf diagrams keep the `EntityImageDegenerated` `delta(14,14)` path. F4 fixtures now jar-exact (cifaki 190×65); census 6→12 conformant. |
