...
const FIXTURES_DIR = join(import.meta.dirname, '../fixtures/state');

// Matched by statePlugin (state keyword) and NOT by sequencePlugin.
...
describe('render() — state diagram async API', () => {
  ...
  it('SVG contains state names', async () => {
    const svg = await render(BASIC_PUML, { measurer: testMeasurer });
