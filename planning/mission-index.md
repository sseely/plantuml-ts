...
| G2 | class SVG conformance | done | G1 (done) | 100% conformant minus known divergences (ruling 2026-07-14). **DONE 2026-07-18** — `plans/g2-class-svg/` (branch `feat/g2-class-svg`, 68 iterations N0–N67): corrected baseline 0/718 (N0 census-pipeline bug; oracle cache re-captured N3 under the frozen-DOT go/no-go) → **292/718 byte-conformant**, class ratchet 0 → **292 pinned**; exit bar MET — all 426 non-conformant fixtures named (ledger § N67 breakdown). Landed: document shell, uid scheme w/ phantom burns, box chrome + badges + HeaderLayout centering, the whole note subsystem, creole in members/notes, URL wrapping, folder/rect namespaces w/ ink dispatch, full edge machinery (labels/ids/decor/reversal/manualArrowheads), style cascades (.tagname/snapshots/conditional/monochrome), MaximumWidth wrap, hide/show family. Remaining named residue: **gvts-blocked ~288 (67.6% — five verified graphviz-ts items w/ repros; getLayout-vs-render splines dominant; library fixes are the largest census unlock anywhere)**; maintainer-scoping: groupInheritance 9, dotted-namespace 13-43, item-20 port-tables 2; mode-dark 1; in-repo singleton tail ~85-128. DOT gate frozen exact + description 48-set intact through all 68 iterations; 9,818 tests. |
| G3 | object SVG conformance | in-progress (started 2026-07-19, maintainer "start G3") | G2 (done) | 100% conformant minus known divergences (ruling 2026-07-14) — `plans/g3-object-svg/` (branch `feat/g3-object-svg`); 80-fixture corpus, oracle cache verified fresh (2026-07-11, post-deterministic-text); object rides the class engine, so G2's subsystems inherit; new standing gate: the 292-fixture class zero-diff set must stay intact |
