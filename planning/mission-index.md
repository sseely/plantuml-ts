...
| SI7 | pipeline order: split `@start…@end` **before** TIM | todo | SI5a | `extractBlocks` runs on near-raw lines; TIM preprocesses each block's *interior*. `buveco-86-tibo673` renders. | **Architectural divergence found 2026-07-13 (SI6 verification).** Our order is `preprocess() → extractBlocks()`. **Upstream's is the reverse**, and the distinction is load-bearing: `BlockUmlBuilder` reads through `preproc2.Preprocessor` — which does **only** config-injection + line-merging, **NOT** TIM — so `@start`/`@end` are detected on near-raw lines and **cannot be swallowed by a conditional**. TIM (`TContext`, `!ifdef`) then runs on the block's *interior*. Because we invert this, a conditional can eat our `@enduml`: `buveco-86-tibo673` (an unclosed `!ifdef`, itself a PlantUML bug report — forum.plantuml.net/6808) loses its `@enduml`, so `extractBlocks` finds no block and we emit "no diagram found" where the jar renders the title. **Do NOT patch this in `extractBlocks` by closing unclosed blocks at EOF** — that is a symptom fix at the wrong layer and gives a *new* wrong answer on a *false* unclosed `!ifdef` (where the jar correctly reports an empty document). Mirror upstream's order instead. Also needs a **default diagram type** for a content-less `@startuml` (jar → `CLASS`; we → "unknown diagram type"). **Impact: 1 of 5,694 pdiff fixtures; 0 of the 1,428 DOT-gating fixtures** — real but not urgent. |
