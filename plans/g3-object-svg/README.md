...
| O3+ | Drill remaining family-table items largest-first (excluding the 41/80 gvts-attributed path/@d family and the newly-reclassified `gatefi-65-curu360` gvts item): the `hide object fields` directive alias (`beruju-17-jigi548`), unicode-identifier + reversed-arrow rank-order divergence (`beleso-08-ruca459`/`fikojo-87-tine499`/`sarepa-89-cevi460`, HIGH-VALUE but HIGH-RISK — touches the shared `class-arrow-grammar.ts#resolveArrow` swapDirection mechanism used by class/component/state too, needs its own dedicated, carefully-gated iteration), the `<<tag>> { FontSize }` skinparam cascade, table-syntax-in-object-body (`pikuba-31-faxo766`), `<style> json/map { MaximumWidth/MinimumWidth/Margin/Padding }` (`maxosa-84-juci042`), `!procedure`-nested-diagram-in-legend (`zuvila-56-nuda425`) | todo |
