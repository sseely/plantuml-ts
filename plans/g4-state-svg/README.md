...
- `state` census: **14/271** zero-diff (`1-3:29, 4-10:136, 11-30:44, 31+:48,
  ...
  all 13 previously-pinned fixtures verified unchanged, +1 new pin
  (`tezivo-82-rufa055`), `conformant && dotEqual`.
...
- `state-dot-parity.test.ts` (size-backlog ratchet): **268/268** passing
  ...
  render-structure-only, no sizing-formula changes.
- `npm test -- --run`: 9970/9970 passing (366 files), `npm run
  typecheck` / `npm run lint` / `npm run build`: all clean.
- `state.golden.ratchet.test.ts`: 16 tests (14 pins), up from 15 (13 pins).

## Mission status (S5, 2026-07-20)

**OPEN.** Landed mechanism 9 (composite internal-transition nesting — the
mission's own stated primary scope) in full: jar nests a composite pass's
own internal transitions INSIDE that pass's own `<g>`, never as flat
top-level siblings — replaces the S1-era documented simplification,
jar-verified against `bajelo-54-dixe684`'s full document structure via a
pretty-printed XML dump. Landing it triggered the SAME mixed-direction
unmasking pattern S0/S1/S3/S4 each already exhibited: closing the top-level
childCount short-circuit let `compareSvg` recurse deeper into composite
fixtures, surfacing THREE further mechanisms — two landed this iteration
(mechanism 10, top-level real-before-pseudo ordering, partial; mechanism 11,
`EntityImageStateEmptyDescription` for `hide empty description` + zero body
lines, in full, +1 pin), one more (mechanism 12, an explicit background
`<rect>` for non-default diagram backgrounds) discovered independently
during the mandated 1-3-bucket sampling pass and landed too. Two more
(mechanisms 13/14, concurrent-region separator lines never drawn; a
per-region pseudo-node id collision) were diagnosed via a full jar XML
pretty-print but NOT landed this iteration — genuinely new rendering
features rather than cheap formula fixes, explicitly deferred per this
iteration's own "cheapest first" instruction. Net census: 13/271 → 14/271
zero-diff, with the underlying bucket distribution shifting toward larger
(but now real, individually addressable) diff counts — the same trade every
prior major-mechanism iteration made. The mission's own secondary scope item
(transition `path/@d` routing/positioning) was NOT started this iteration —
mechanism 9's own scope plus its unmasking cascade consumed the full
iteration budget. See `plans/g4-state-svg/ledger.md` S5 for the full
mechanism writeups, the hand-sampled attribution table, and the S6+ queue.
