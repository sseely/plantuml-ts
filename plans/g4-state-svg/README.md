...
- `state` census: **9/271** zero-diff (`1-3:17, 4-10:182, 11-30:40,
  31+:23, errors:0`) — held from S2's `9/271` (`1-3:18, 4-10:187,
  11-30:37, 31+:20`); the SAME fixtures pinned to the ratchet
  (`conformant && dotEqual`), verified unchanged.
- Class census: **303/718**, intact, unchanged (bumped from 294 by the
  orchestrator's own graphviz-ts/`DotInputNode.shape` fix between S2/S3,
...
- DOT gate: `component 262/262 - usecase 90/90 - class 708/708 - object
  ...
  after every mechanism landed/attempted this iteration).
- `npm test -- --run`: 9955/9955 passing (364 files), `npm run
  typecheck` / `npm run lint` / `npm run build`: all clean.

## Mission status (S3, 2026-07-20)

**OPEN.** Mechanism 6 (composite box rendering convention) landed for the
AUTONOM (`entity`-wrap) case — the real 3-4-layer shape (half-rounded
header path + solid outline + divider(s) + centered title + optional
action-zone bg/text) is now jar-verified byte-exact, replacing the S1-era
dashed-rect approximation. Closing the composite `childCount` mismatch
surfaced a SEVENTH mechanism (composite wrapper width/height sizing gap —
`measureAutonomWrapper`'s child-image parameter uses the wrong `SvekResult`
formula); a trial fix was jar-verified to help 2 fixtures but ALSO
jar-verified to regress 2 already-pinned `size-backlog.json` entries past
their tighten-only allowance, so it was REVERTED and queued whole for S4
(needs combining with a still-separate child position-offset residual —
full diagnosis in the ledger). Composite ink-extent (item 2) shows strong
algebraic evidence of no independent bug but isn't yet independently
verifiable (blocked on mechanism 7 landing); the entity-vs-cluster wrap
split (item 3) was confirmed genuinely unbounded this iteration (a
DOT-native cluster-label sizing path needing library-level cluster-bbox
exposure). Zero-diff count held at 9/271 — the SAME "unmasking" bucket
churn (some fixtures move to worse buckets as previously-hidden structure
becomes visible) this mission has exhibited at every mechanism-landing
iteration since S0→S1. See `plans/g4-state-svg/ledger.md` S3 for full
mechanism writeups and the S4+ queue.
