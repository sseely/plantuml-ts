...
| I5b | entity/link multi-stereotype: only the FIRST `<<tag>>` renders, upstream stacks one `<text>` per tag | 12 | `extractNodeStereotype`'s own doc comment already named this as deferred ("a text-metric detail"); `DescriptiveNode.stereotype` needs `string[]`, `buildStereo` (EntityImageDescriptionSupport.ts) already supports N lines once fed an array |
| I5c | bracket-body `[Line1\nLine2]` shorthand: literal `\n` not resolved via `finalizeDisplay` for that parse path | 2+ | `saxosu-09-nodi002`/`seguci-13-zure968` — I4c's `finalizeDisplay` wiring doesn't reach the `[...]`-bracket declaration grammar |
| I5d | transparent/near-zero-alpha color (`FontColor transparent`, `BackgroundColor transparent`, `#00000000`) draws the element instead of eliding it | ~14 fixtures / 37 diff instances (25 text + 12 rect) | jar's `SvgGraphics`/`XmlWriter` skips emitting a `<text>`/`<rect>` entirely when the resolved color is fully transparent; this port always emits it |
| I5e | link-endpoint auto-create stereotype (`Name<<tag>>` on the arrow's target) wrongly drawn as the LINK's own visible `«tag»` label | ~6 | the tag configures arrow skinparam/style selectors (`skinparam Arrow{Color<<tag>>}`) and the AUTO-CREATED entity's stereotype — it is never itself a link label upstream |
