...
- **item 20** (enhanced-body member-row EDGE-port anchor exposure, 2+
  reach, "genuinely new subsystem" per N48/N55) — `enhanced-body-member`
  tag shows 32 heuristic reach this iteration (up from N48's 17 — the tag
  itself over-matches, see its own regex; real item-20-shaped reach is a
  SUBSET, unconfirmed without a ground-truth pass) — unattempted.
...
- **item 40** (`strictuml` class-icon suppression): `fogexa`'s remaining
  `svg/@height`/`@viewBox[3]` diff (183 vs jar's 175, delta 8) traces to
  the CLASS box itself, not the note — jar's plain-UML box under
  `strictuml` omits the badge ellipse/glyph icon entirely (a narrower box,
  `height=40` vs this port's `height=48`, which still draws the green
  circled-"C" badge); unverified against upstream source this iteration
  (budget spent on items 37/38's own to-HARD-BOUNDARY verification) —
  named, not diagnosed to mechanism yet. Distinct from the already-tracked
  `circledCharFontSize`/`circledCharFontStyle` tags (9+9 reach, N56's own
  accounting) and the `strictuml` tag itself (6 reach) — may be the SAME
  root mechanism as one or both, unconfirmed.
