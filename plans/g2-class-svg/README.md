...
| N67 | Item 49 (class-diagram note `<style> note { FontColor N } }` cascade) LANDED: new `theme.colors.graph.noteCascadeFontColor` field, SAME `NOTE_SNAMES` signature N66 built for `MaximumWidth` (`{root,element,classdiagram,note}`), wired via `cascadeFontColorHex` (item 29's own FontColor-specific helper, including the `#?light:dark` conditional path against a new `DEFAULT_NOTE_BACKGROUND` estimate). `renderer-note.ts#renderNoteLineAtoms`/`renderNoteText` now consult it as a fallback tier below an atom's own explicit `<color>` run and above the hardcoded `#000000` default. Jar-verified BYTE-EXACT against `nufini-44-jofo787` (24 note-text `fill` diffs -> 0, `diffCount: 30 -> 6`, residual 6 a SEPARATE pre-existing entity-ordering gap, not attempted). Near-zero harvest (27-fixture 1-3 bucket, full classification): LANDED a NEW mechanism -- `xabije-20-xusi569`'s visibility-icon Y-centering used the diagram-global `theme.fontSize` instead of the row's own resolved `skinparam class { AttributeFontSize N }` (`renderer-classifier-box.ts`'s two `visibilityIconOriginY` call sites), a 1.1111px descent-formula delta jar-verified byte-exact (new `attributeFontSize(theme)` helper, mirrors `class-layout-helpers.ts`'s own `attributeFont.size` formula). Remaining 26 fixtures fully classified by direct source read (not diff-shape guessing): 8 re-confirmed already-declined, 4 confirmed `skinparam groupInheritance` (awaiting-maintainer), 7 canvas-dims fixtures DRILLED with real actual/expected numbers and CONFIRMED heterogeneous (deltas span 1-200px, no shared formula, several combine a genuine `childCount` structural diff alongside the numeric one), 5 NEW leads precisely named but not landed (`sejuzo-42-fini523`'s dividerYs/inline-leading-url interaction; a candidate shared mechanism across `danozo-79-nunu375`/`vudepo-27-cuvo793`/`lejoga-79-poji465`'s note-left/right + redeclaration-or-extended-arrow shape, unconfirmed; `rizexu-84-xujo903` matching the known enhanced-body family; `gadufu-56-votu808`'s Cyrillic+embedded-activity-block syntax; `bujedi-30-cize673`'s `linetype ortho` spline routing, graph-layout-adjacent). Strategic survey of all 426 non-conformant fixtures produced for the orchestrator (full breakdown in `ledger.md` N67): ~288/718 (67.6%) gvts-blocked (FRESH `--families` scan, `svg/g/g/path/@d` reach, stable since N62's 286 six iterations ago, subsumes 5 named library-level sub-mechanisms), ~9-43 DOT-topology-awaiting-maintainer (methodology gap stated explicitly -- two different measurement methods disagree, not papered over), 1 mode-dark ColorMapper, remainder (~85-128 by subtraction) a long tail of 1-2-fixture-reach genuinely-tractable mechanisms -- diminishing per-iteration returns are now structural, not a triage failure. 12 new unit tests across 3 files, TDD, jar-verified where a golden exists. Full-corpus regression scan (disposable worktree, per-fixture zero-diff SET diffed, not just aggregate buckets): 0 regressions, 1 new zero-diff. DOT gate + description gate re-verified EXACTLY unchanged. | **1 new zero-diff** (`xabije-20-xusi569`); census 292/718 (was 291) · 1-3:26 (was 27) · 4-10:105 (was 104) · 11-30:29 (was 30) · 31+:266 (unchanged); ratchet 292 fixtures / 294 tests (was 291/293) | done |
...
35. **`<style> class { MaximumWidth N } }` text-wrap cascade** --
    **N65: LANDED for the header + member-row call sites** (see
    `ledger.md` N65 for the full jar-verified derivation; a note-text
    call site remains unattempted, named separately below). Original
    N49 finding (NEWLY
    ...
    **N64: mechanism fully derived, NOT landed** — `Style.java:292-295`
    (`Style#wrapWidth`) feeds a REAL (non-`NONE`) `LineBreakStrategy` into
    the SAME `Display#create8` call item 45 uses, so `Fission#getSplitted`
    (`klimt/creole/Fission.java:63-101`) actually runs: a word-boundary
    greedy-wrap algorithm needing NEW `Neutron`/`NeutronType` primitives
    this codebase doesn't have at all (zero-hit grep), wired through BOTH
    the header AND a separate, untraced member-row call site, plus the
    mission's own flagged `dot-sync-report.ts` empirical DOT-dimension
    re-check (unlike items 43/44's presence-only `labelOk` safety). Full
    derivation in `ledger.md` N64 for direct reuse — matches item
    20/39/monochrome/gradient-color's own declining precedent for a
    newly-ground-truthed multi-subsystem scope on a 2-3 fixture reach
    gap.
...
- **mode-dark ColorMapper subsystem**: `skinparam mode dark` (1 tagged
  reach). **N62: fully re-diagnosed from source** (not just re-asserted) —
  confirmed NOT superseded by N61's `monochrome` pass: dark mode is a
  discrete, independently-authored ~20-selector lookup table resolved
  through the FULL style cascade (`plantuml.skin`'s trailing `@media
  (prefers-color-scheme:dark)` block, `StyleParser`'s mode-switch token
  pairing light/dark `Value`s on the SAME `Style`, `HColor
  #darkSchemeTheme()` reading the paired field), categorically different
  from `MONOCHROME`'s single post-hoc grayscale formula — a genuine new
  subsystem (parallel style-cascade resolution, not a chokepoint regex
  extension), correctly declined for the same reason N58/N60/N61 already
  gave chokepoint-level color features on a 1-fixture reach figure. Full
  mechanism + file:line citations in `ledger.md` N62. Still unattempted.
...
- **item 45 — RESOLVED N64, drop from future queues** (classifier
  display-name `\n`/`\l`/`\r` line-wrapping): LANDED, reusing
  `splitEdgeLabelLines` verbatim — new `class-stereotype.ts#buildHeaderRows`
  + `ClassifierGeo.nameRowCount`, jar-verified byte-exact against
  `dofima-22-kofe334`/`jireze-84-loti743`. A jar-verified NBSP-substitution
  corollary (blank split line after generic-tag extraction) landed
  alongside it. Full derivation + regression-scan diagnosis in
  `ledger.md` N64.
- **item 46 — RESOLVED N64, drop from future queues** (plain-label
  quote-stripping gap): LANDED — `class-relationship-parser.ts`'s label
  resolution now mirrors `Labels#init`'s unconditional fallthrough
  `stripQuotes` call, jar-verified against both the no-quantifier
  (`begico-70-guva302`) and explicit-quantifier (`pucazu-91-paxe635`)
  sub-cases. Full derivation in `ledger.md` N64.
...
### New items surfaced N64 (found via items 45/46's own full-corpus
### regression scan, per-fixture diagnosed — neither landed this
### iteration, both genuinely separate from items 45/46's own scope)

- **item 47** (`skinparam RoundCorner N`, top-level/non-tag-scoped
  resolution gap) — **RESOLVED N65**, see the new N65 table row below;
  drop from future queues.
- **item 48** (creole/icon markup inside a classifier's quoted display
  name): `<color:...>`/`<plain>`/OpenIconic `<:name:>` tags embedded
  directly in a `class "..."` alias (e.g. `enum BookCategory as
  "<color:#888888><plain>Enumeration</plain></color>\nBookCategory"`)
  render as literal escaped tag text — jar's golden shows the tags
  INTERPRETED (per-run color/glyph substitution, 2 differently-styled
  `<text>` rows). The classifier-header render path has never routed
  through `class-member-creole.ts`'s atom engine at all — a genuinely new
  subsystem (parsing + per-run styled `<text>` emission), not a wiring
  gap. Reach ≥ 2/718 (`diseka-11-gozu390`, `lecelo-92-loma110`), found via
  N64's own regression scan. **N65: ALSO confirmed to cover simple
  `**bold**`/`__underline__` markdown** (not just `<color:>`/OpenIconic —
  jar's header path always runs the FULL creole engine, `CreoleMode
  .FULL_BUT_UNDERSCORE`, wrap or no wrap; `nucite-98-kuga991`'s own
  `**class**` header suffix renders as literal asterisks in this port,
  bold in jar). Not attempted, matches item
  20/39/monochrome/gradient-color's own declining precedent.
- **`scale N` directive** (`koxoco-29-moke425`): a post-layout uniform
  canvas/attribute scaling transform (font-size/stroke-width/every
  coordinate multiplied by the scale factor) is entirely unimplemented —
  confirmed via direct render comparison (jar's golden scales every
  numeric attribute by 0.8, this port's render scales nothing). Reach ≥
  1/718, an implicit pre-existing gap newly named with a concrete
