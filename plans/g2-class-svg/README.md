...
- **item 47** (`skinparam RoundCorner N`, top-level/non-tag-scoped
  resolution gap): `renderer-classifier-box.ts#buildHeaderPrimitive`'s
  `roundCorner` resolution only consults `resolveClassTagCascadeEntry`
  (a `<style>` tag-scoped override) and a hardcoded default — a bare
  top-level `skinparam RoundCorner 20` (no `<style>` block, no tag
  qualifier) is never read at all. Reach ≥ 1/718 (`dofima-22-kofe334`'s
  `rect/@rx`/`@ry`, 2.5 vs jar's real 10), found via N64's own regression
  scan, not corpus-surveyed. Confirmed zero code-path overlap with item 45.
- **item 48** (creole/icon markup inside a classifier's quoted display
  ...
  N64's own regression scan. Not attempted, matches item
