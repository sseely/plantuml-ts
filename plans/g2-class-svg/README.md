...
| N2 | mechanism 3: entity/cluster/link `<g>` wrapping + `ent%04d`/`lnk%d` uid assignment (universal, 0/718 spared — `ledger.md` N1) — the NEW primary blocker, likely bundled with or prerequisite to the width/height/viewBox geometry divergences (717/713/718, several distinct sub-mechanisms, not yet decomposed) and the 16-fixture `defs[childCount]` residual | TBD | todo |
