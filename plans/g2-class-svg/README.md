...
| N3 | `EntityImageClass` chrome-fidelity pass: classifier box padding/rounding (`x/y/rx/ry/width/height`, jar's `x=7,y=7,rx=2.5,ry=2.5` vs this port's flush `x=0,y=0` unrounded rect — `svg/g/g/rect/@*` families, 50-62 fixtures each), the kind-badge real vector icon (jar draws `<ellipse>`+bezier-`<path>` glyphs, e.g. "C"/"I"/"A"/"E"/"@" — this port draws `<circle>`+`<text>`, `svg/g/g/circle` 48 fixtures), and likely generics/type-param icon shapes (`bedogi-86-kala547`'s `<T>` box). Smaller riders if time remains: edge `<path id/codeLine>` attrs (`renderer.ts#buildPathData`, `ledger.md` N2), the `muteClassifierToGroup` off-by-one uid bug (`class-container.ts`, `ledger.md` N2), `data-source-line` threading (parser-wide, comparable to description's own I3b — lower priority, `data-*` is comparison-stripped). | TBD | todo |
